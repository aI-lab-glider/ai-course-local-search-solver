...
class AddTriangle(Move[AvatarState]):
    def __init__(self, from_state: AvatarState, vertices: List[Vertex], color: Color):
        ...
        (self.coordinates, self.color) = self._to_coordinates(vertices), self._to_color_coordinates(color)
    ...
    def make(self) -> AvatarState:
        ...
        new_image_draw.polygon(self.coordinates, fill=self.color, outline=self.color)
        return AvatarState(model=self.state.model, image=new_image)
